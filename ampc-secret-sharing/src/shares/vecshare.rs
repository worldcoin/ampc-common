...
use std::ops::{
    Add, AddAssign, BitXor, BitXorAssign, Deref, DerefMut, Mul, MulAssign, Neg, Not, Sub, SubAssign,
};

#[derive(Clone, Copy, Debug)]
#[repr(transparent)]
pub struct SliceShare<'a, T: IntRing2k> {
    shares: &'a [ReplicatedShare<T>],
}
...
impl<'a, T: IntRing2k> SliceShare<'a, T> {
    ...
    pub fn chunks(&self, chunk_size: usize) -> impl Iterator<Item = SliceShare<'_, T>> + '_ {
        self.shares
            .chunks(chunk_size)
    ...
    pub fn iter(&self) -> std::slice::Iter<'_, ReplicatedShare<T>> {
        self.shares.iter()
    }
...
pub struct SliceShareMut<'a, T: IntRing2k> {
    shares: &'a mut [ReplicatedShare<T>],
}
...
impl<'a, T: IntRing2k> SliceShareMut<'a, T> {
    ...
    pub fn to_slice(&self) -> SliceShare<'_, T> {
        SliceShare {
            shares: self.shares,
        }
    }
...
#[repr(transparent)]
pub struct VecShareAdditivePrime<T: PrimInt> {
    pub shares: Vec<AdditiveSharePrime<PrimeElement<T>>>,
}

impl<T: PrimInt> VecShareAdditivePrime<T> {
    #[cfg(test)]
    pub fn new_share(share: AdditiveSharePrime<PrimeElement<T>>) -> Self {
        let shares = vec![share];
        Self { shares }
    }

    /// Access the underlying shares
    pub fn shares(&self) -> &[AdditiveSharePrime<PrimeElement<T>>] {
        &self.shares
    }
    ...
    pub fn inner(self) -> Vec<AdditiveSharePrime<PrimeElement<T>>> {
        self.shares
    }
...
impl<T: IntRing2k> VecShareReplicated<T> {
    ...
    /// Access the underlying shares
    pub fn shares(&self) -> &[ReplicatedShare<T>] {
        &self.shares
    }

    pub fn new_vec(shares: Vec<ReplicatedShare<T>>) -> Self {
        Self { shares }
    }

    pub fn inner(self) -> Vec<ReplicatedShare<T>> {
        self.shares
    }
    ...
    pub fn iter_mut(&mut self) -> std::slice::IterMut<'_, ReplicatedShare<T>> {
    ...
    pub fn push(&mut self, el: ReplicatedShare<T>) {
        self.shares.push(el)
    }

    pub fn pop(&mut self) -> Option<ReplicatedShare<T>> {
        self.shares.pop()
    }
    ...
    pub fn split_at_mut(&mut self, mid: usize) -> (SliceShareMut<'_, T>, SliceShareMut<'_, T>) {
        let (a, b) = self.shares.split_at_mut(mid);
        (SliceShareMut { shares: a }, SliceShareMut { shares: b })
    }

    pub fn get_at(&self, index: usize) -> ReplicatedShare<T> {
    ...
    pub fn flatten(inp: Vec<Self>) -> impl Iterator<Item = ReplicatedShare<T>> {
    ...
    pub fn as_slice(&self) -> SliceShare<'_, T> {
        SliceShare {
            shares: &self.shares,
        }
    }

    pub fn as_slice_mut(&mut self) -> SliceShareMut<'_, T> {
        SliceShareMut {
...
impl VecShareReplicated<Bit> {
    ...
    pub fn pack<T: IntRing2k>(self) -> VecShareReplicated<T> {
        ...
        for a_ in self.shares.chunks(T::K) {
            ...
            let share = ReplicatedShare::new(share_a, share_b);
            out.push(share);
        }
...
impl<T: IntRing2k> Sub<Self> for VecShareAdditive<T> {
    type Output = Self;
...
impl<T: IntRing2k> AddAssign<Self> for VecShareAdditive<T> {
    fn add_assign(&mut self, rhs: Self) {
        self.shares
            .iter_mut()
            .zip(rhs.shares.iter())
            .for_each(|(left_share, right_share)| left_share.value += right_share.value);
    }
}
...
impl<T: IntRing2k> SubAssign for VecShareAdditive<T> {
    fn sub_assign(&mut self, rhs: Self) {
        self.shares
            .iter_mut()
            .zip(rhs.shares.iter())
...
impl<T: IntRing2k> SubAssign<&Self> for VecShareAdditive<T> {
    fn sub_assign(&mut self, rhs: &Self) {
        self.shares
            .iter_mut()
            .zip(rhs.shares.iter())
...
impl<T: IntRing2k> Mul<&[RingElement<T>]> for VecShareAdditive<T> {
    type Output = Self;
    ...
    fn mul(self, rhs: &[RingElement<T>]) -> Self::Output {
        let mul_shares = self
            ...
            .iter()
            .zip(rhs.iter())
            .map(|(left_share, rhs_mul)| AdditiveShare {
                value: left_share.value * rhs_mul,
            })
            .collect();
...
impl<T: IntRing2k> Mul<&[T]> for VecShareAdditive<T> {
    type Output = Self;
...
impl<T: IntRing2k> MulAssign<Vec<T>> for VecShareAdditive<T> {
    fn mul_assign(&mut self, rhs: Vec<T>) {
        self.shares
            .iter_mut()
            .zip(rhs.iter())
            .for_each(|(left_share, rhs_mul)| left_share.value *= rhs_mul);
    }
}

impl<T: IntRing2k> BitXor<Self> for &VecShareAdditive<T> {
    type Output = VecShareAdditive<T>;
    ...
    fn bitxor(self, rhs: Self) -> Self::Output {
        let xor_shares = self
            ...
            .collect();
        VecShareAdditive { shares: xor_shares }
    }
}
...
impl<T: IntRing2k> BitXorAssign<&Self> for VecShareAdditive<T> {
    fn bitxor_assign(&mut self, rhs: &Self) {
        self.shares
            .iter_mut()
            .zip(rhs.shares.iter())
...
impl<T: IntRing2k> Neg for VecShareAdditive<T> {
    type Output = Self;

    fn neg(self) -> Self::Output {
        let neg_shares = self.shares.iter().map(|share| -share).collect();
        VecShareAdditive { shares: neg_shares }
    }
}
...
impl<T: PrimInt> Add<&Self> for VecShareAdditivePrime<T> {
    type Output = Self;
...
impl<T: PrimInt> Sub<Self> for VecShareAdditivePrime<T> {
    type Output = Self;
...
impl<T: PrimInt> AddAssign<Self> for VecShareAdditivePrime<T> {
    fn add_assign(&mut self, rhs: Self) {
        self.shares
            .iter_mut()
            .zip(rhs.shares.iter())
            .for_each(|(lhs, rhs)| lhs.value += rhs.value);
    }
}
...
impl<T: PrimInt> SubAssign for VecShareAdditivePrime<T> {
    fn sub_assign(&mut self, rhs: Self) {
        self.shares
            .iter_mut()
            .zip(rhs.shares.iter())
...
impl<T: PrimInt> SubAssign<&Self> for VecShareAdditivePrime<T> {
    fn sub_assign(&mut self, rhs: &Self) {
        self.shares
            .iter_mut()
            .zip(rhs.shares.iter())
...
impl<T: PrimInt> Mul<Vec<PrimeElement<T>>> for VecShareAdditivePrime<T> {
    type Output = Self;

    fn mul(self, rhs: Vec<PrimeElement<T>>) -> Self::Output {
        let shares = self
            .shares
            .iter()
            .zip(rhs.iter())
...
impl<T: PrimInt> Mul<Vec<PrimeElement<T>>> for &VecShareAdditivePrime<T> {
    type Output = VecShareAdditivePrime<T>;
...
impl<T: PrimInt> MulAssign<Vec<PrimeElement<T>>> for VecShareAdditivePrime<T> {
    fn mul_assign(&mut self, rhs: Vec<PrimeElement<T>>) {
        self.shares
            .iter_mut()
            .zip(rhs.iter())
            .for_each(|(lhs, rhs)| lhs.value *= *rhs);
    }
}

impl<T: PrimInt> Neg for VecShareAdditivePrime<T> {
    type Output = Self;

    fn neg(self) -> Self::Output {
        let shares = self.shares.iter().map(|lhs| -lhs).collect();
        VecShareAdditivePrime { shares }
    }
}
...
mod tests {
        ...
        Standard: Distribution<RingElement<T>>,
    {
        ...
        let mut shares = vec![ReplicatedShare::zero()];
        let one_share = ReplicatedShare::new(RingElement::one(), RingElement::one());
        for i in 1..17 {
