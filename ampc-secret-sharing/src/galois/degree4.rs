...
pub struct GaloisRingElement<B: basis::Basis> {
    pub coefs: [u16; 4],
    basis: PhantomData<B>,
}

impl GaloisRingElement<basis::Monomial> {
    pub const ZERO: GaloisRingElement<basis::Monomial> = GaloisRingElement {
        coefs: [0, 0, 0, 0],
        basis: PhantomData,
    };
    pub const ONE: GaloisRingElement<basis::Monomial> = GaloisRingElement {
        coefs: [1, 0, 0, 0],
        basis: PhantomData,
    };
    pub const EXCEPTIONAL_SEQUENCE: [GaloisRingElement<basis::Monomial>; 16] = [
...
impl<B: basis::Basis> std::ops::Add for GaloisRingElement<B> {
    type Output = Self;
    fn add(self, rhs: Self) -> Self::Output {
        self.add(&rhs)
    }
}
impl<B: basis::Basis> std::ops::Add<&GaloisRingElement<B>> for GaloisRingElement<B> {
    type Output = Self;
...
impl<B: basis::Basis> std::ops::Sub for GaloisRingElement<B> {
    type Output = Self;
    fn sub(self, rhs: Self) -> Self::Output {
        self.sub(&rhs)
    }
}
impl<B: basis::Basis> std::ops::Sub<&GaloisRingElement<B>> for GaloisRingElement<B> {
    type Output = Self;
...
impl<B: basis::Basis> std::ops::Neg for GaloisRingElement<B> {
    type Output = Self;
...
impl std::ops::Mul for GaloisRingElement<basis::Monomial> {
    type Output = Self;
    fn mul(self, rhs: Self) -> Self::Output {
        self.mul(&rhs)
    }
}
impl std::ops::Mul<&GaloisRingElement<basis::Monomial>> for GaloisRingElement<basis::Monomial> {
    type Output = Self;
    fn mul(self, rhs: &Self) -> Self::Output {
        GaloisRingElement {
            coefs: [
...
impl GaloisRingElement<basis::A> {
    pub fn to_monomial(&self) -> GaloisRingElement<basis::Monomial> {
...
impl<B: basis::Basis> GaloisRingElement<B> {
...
pub struct ShamirGaloisRingShare {
    pub id: usize,
    pub y: GaloisRingElement<basis::Monomial>,
}
...
impl ShamirGaloisRingShare {
    pub fn encode_3<R: CryptoRng + Rng>(
        input: &GaloisRingElement<basis::Monomial>,
        rng: &mut R,
    ...
    pub fn deg_1_lagrange_polys_at_zero(
        my_id: PartyID,
        other_id: PartyID,
    ) -> GaloisRingElement<basis::Monomial> {
    ...
    pub fn deg_1_lagrange_poly_at_v(
        ...
        let v = v + 1;
        let mut res =
            GaloisRingElement::EXCEPTIONAL_SEQUENCE[v] - GaloisRingElement::EXCEPTIONAL_SEQUENCE[j];
        res = res
            * (GaloisRingElement::EXCEPTIONAL_SEQUENCE[i]
                - GaloisRingElement::EXCEPTIONAL_SEQUENCE[j])
                .inverse();
    ...
    pub fn deg_2_lagrange_polys_at_zero() -> [GaloisRingElement<basis::Monomial>; 3] {
...
mod tests {
    ...
    fn sharing_mat() {
        ...
        let shares1 = ShamirGaloisRingShare::encode_3_mat(&input1.coefs, &mut rand::thread_rng());
        let shares2 = ShamirGaloisRingShare::encode_3_mat(&input2.coefs, &mut rand::thread_rng());
        let shares_mul = [
            shares1[0] * shares2[0],
            shares1[1] * shares2[1],
            shares1[2] * shares2[2],
    ...
    fn basis_conversions() {
        ...
        let input1 = GaloisRingElement::<super::basis::A>::random(&mut rand::thread_rng());
        let input2 = GaloisRingElement::<super::basis::A>::random(&mut rand::thread_rng());
