...
fn test_bit_inject(
    ...
    let s1 = ReplicatedShare::iter_from_iter_ab(input1.into_iter(), mine.into_iter());
    // - s2 = (0, 0) of [b_2] (we can ignore this shares as they are zero)
    // - s3 = (0, y) of [r_01 * b_2]
    let s3 = ReplicatedShare::iter_from_iter_ab(zero_iter(len), input2.into_iter());
    // - s4 = (r_12, 0) of [x * b_2]
    let s4 = ReplicatedShare::iter_from_iter_ab(theirs.into_iter(), zero_iter(len));
