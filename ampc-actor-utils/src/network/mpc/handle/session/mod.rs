...
impl Fragment {
    fn encode(
        ...
        Ok(NetworkValue::Bytes(bytes.into()))
    }
    ...
    fn decode(value: NetworkValue) -> Result<Self> {
        ...
        Ok(Self {
            ...
            index,
            count,
            payload: bytes.slice(STRIPE_HEADER_BYTES..),
        })
    }
}
...
impl Assembly {
    ...
    fn assemble(self) -> Result<NetworkValue> {
        ...
        for chunk in self.chunks {
            let chunk = chunk.ok_or_else(|| eyre!("missing striped MPC fragment"))?;
...
mod tests {
    ...
    async fn out_of_order_stripes_preserve_logical_message_order() -> Result<()> {
        ...
        let first = NetworkValue::Bytes(vec![1; 1024 * 1024].into());
        let second = NetworkValue::Bytes(vec![2; 1024 * 1024].into());
