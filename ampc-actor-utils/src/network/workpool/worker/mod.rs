...
pub async fn build_worker_handle(
    ...
    let worker_addr: SocketAddr = args
        .worker_address
        .parse()
        .map_err(|e: std::net::AddrParseError| WorkerError::ParseError(e.to_string()))?;
    ...
    let job_rx =
        if let Some(tls) = args.tls.as_ref() {
            ...
            let private_key = tls.private_key.as_ref().ok_or_else(|| {
                WorkerError::ConnectionError("TLS private key required".to_string())
            })?;
            let leaf_cert = tls.leaf_cert.as_ref().ok_or_else(|| {
                WorkerError::ConnectionError("TLS leaf cert required".to_string())
            })?;

            let listener = TlsServer::new(worker_addr, private_key, leaf_cert, &root_certs)
                .await
                .map_err(|e| {
                    WorkerError::ConnectionError(format!("Failed to create TLS server: {}", e))
                })?;
            let connector = TlsClient::new_with_ca_certs(&root_certs)
                .await
                .map_err(|e| {
                    WorkerError::ConnectionError(format!("Failed to create TLS client: {}", e))
                })?;

            let leader = crate::network::tcp::Peer::new(args.leader_id, args.leader_address);
            ...
            let listener = BoxTcpServer(TcpServer::new(worker_addr).await.map_err(|e| {
                WorkerError::ConnectionError(format!("Failed to create TCP server: {}", e))
            })?);
