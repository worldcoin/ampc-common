...
use crate::{
    ...
    protocol::{
        ...
        ops::{
            batch_signed_lift_vec_ring48, conditionally_select_distance, reshare_products,
            DistancePair, B,
        },
    },
};
...
pub fn nhd_comparison_nmr(hd: u16, ml: u16) -> i64 {
...
mod tests {
    ...
    fn test_nhd_plaintext_is_match() {
        ...
        let test_cases: Vec<(u16, u16, bool)> = vec![
            ...
            (900, 3000, true), // nhd = 0.36 -> match
            (9, 30, false),    // nhd = 0.38 -> no match
            // Edge cases
    ...
    async fn test_nhd_cross_compare() {
        ...
        for (i, session) in sessions.into_iter().enumerate() {
            ...
            jobs.spawn(async move {
                ...
                let pairs: Vec<DistancePair<Ring48>> = (0..n)
                    .map(|j| {
