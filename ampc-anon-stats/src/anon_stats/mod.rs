...
pub async fn process_2d_anon_stats_job(
    ...
    let job_size = job.len();
    let job_data = job.into_bundles();
    let translated_thresholds = calculate_threshold_a(config.n_buckets_2d);
    ...
    let mut anon_stats = BucketStatistics2D::new(job_size, config.n_buckets_2d, config.party_id, AnonStatsResultSource::Aggregator);
    anon_stats.fill_buckets(&buckets, MATCH_THRESHOLD_RATIO, None);
    anon_stats.source = AnonStatsResultSource::Aggregator;
    Ok(anon_stats)
}
