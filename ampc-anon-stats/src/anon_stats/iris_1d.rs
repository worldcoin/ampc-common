...
use ampc_actor_utils::protocol::ops::batch_signed_lift_vec_u64;
...
pub type LiftedU64DistanceBundle1D = Vec<DistanceShare<u64>>;
...
pub async fn lift_bundles_u64_1d(
    session: &mut Session,
    bundles: &[DistanceBundle1D],
) -> Result<Vec<LiftedU64DistanceBundle1D>> {
    let flattened = bundles
        .iter()
        .flat_map(|x| x.iter().flat_map(|y| [y.code_dot, y.mask_dot]))
        .collect_vec();
    let lifted_flattened = batch_signed_lift_vec_u64(session, flattened.clone()).await?;
...
pub async fn process_1d_lifted_anon_stats_score_normalization_job(
    session: &mut Session,
    job: AnonStatsMapping<LiftedU64DistanceBundle1D>,
    origin: &AnonStatsOrigin,
    config: &AnonStatsServerConfig,
    operation: Option<AnonStatsOperation>,
    start_timestamp: Option<DateTime<Utc>>,
...
mod tests {
    ...
    use crate::{
        anon_stats::{
            calculate_iris_threshold_a, iris_1d::test_helper::TestDistances,
            MATCH_THRESHOLD_RATIO_REAUTH,
        },
