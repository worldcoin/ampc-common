use alkali::asymmetric::seal::{Keypair, PrivateKey, PublicKey};
...
async fn validate_keys(
    ...
    let pub_key = if let Some(b64_pub_key) = b64_pub_key {
        ...
        PublicKey::try_from(user_pubkey.as_slice()).expect("Invalid public key")
    } else {
    ...
    let decoded_priv_key = PrivateKey::try_from(user_privkey.as_slice()).unwrap();
...
async fn rotate_keys(
    ...
    let priv_key_str = STANDARD.encode(private_key.as_ref());
    ...
    if dry_run.unwrap_or(false) {
        ...
        let decoded_pub_key = PublicKey::try_from(user_pubkey.as_slice()).unwrap();
...
mod test {
    use super::*;
    use alkali::asymmetric::seal::{self, Seed};

    fn test_keypair() -> Keypair {
        let seed = Seed::try_from([0u8; 32].as_slice()).unwrap();
    ...
    fn test_encrypt_and_decrypt_shares() {
        ...
        let mut encrypted_share = vec![0; share.len() + seal::OVERHEAD_LENGTH];
        seal::encrypt(share.as_bytes(), &keypair.public_key, &mut encrypted_share).unwrap();

        let mut decrypted_share = vec![0; share.len()];
        seal::decrypt(&encrypted_share, &keypair, &mut decrypted_share).unwrap();
